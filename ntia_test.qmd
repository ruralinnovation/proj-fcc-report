---
title: "Testing NTIA eligibility"
date: last-modified
format: 
  html:
    code-fold: true
...
The dataset to produce this comparison was done quickly and process with pgadmin. 
...
select
    ...
    sum(case when category_not_dsl = 'Underserved' then 1 else 0 end) as cnt_underserved_past,
    sum(case when category_not_wireless = 'Underserved' then 1 else 0 end) as cnt_underserved_past,
    sum(case when category_not_dsl_wireless = 'Underserved' then 1 else 0 end) as cnt_underserved_past
from staging.us_ntia
group by geoid_st
```
