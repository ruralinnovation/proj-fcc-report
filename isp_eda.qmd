...
dta[["brand_name_by_provider_id"]] <- sapply(split(isp$brand_name, isp$provider_id),
                                      function(x) length(x))
...
more_frn_than_provider <- subset(isp, frn_by_provider_id  > 1)
...
- `131486` seems to be RiverStreet Networks with various `frn` (13 cases)

- `190233` multiple brand name and frn but seems to be in Texas and  Oklahoma (13)

- `131226` seems to be Fastwyre Broadband divided by technology and states (12 cases)

- `130804` seems to be Mediacom (+ Bolt) with different states and names indicating their states (11 cases) 

- Google Fiber (`240041`) seems to be have `frn` split by states (with a weird 'Webpass, Inc.') (11 cases)

- AT&T Inc (`130077`) multiple frn (10 cases)
